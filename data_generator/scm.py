...
class MultiEnvLatentSCM(ABC):
    ...
    def __init__(
        ...
        self.selected_idx= selected_idx if selected_idx != None else [self.latent_dim-1]
        self.observed_idx = observed_idx if observed_idx != None else [self.latent_dim-2]
        self.idx = selected_idx + observed_idx
        self.num_label = int((2*self.num_blocks))

        self.var_dict = {}
        #self.m_dict = {}

        for i in range(self.num_label):
            #self.m_dict[i] = np.random.uniform(-5,5)
            self.var_dict[i] = np.random.uniform(0.3,1)
    ...
    def push_forward(self, u: Tensor, env: int) -> Tensor:
        ...
        #     zs.append(norm.ppf((i + 1) / (2 * self.num_blocks)))
        # v = torch.nan * torch.zeros_like(u)
        # for k, index in enumerate(self.topological_order):
        #     if k == 0:
        #         for j, z in enumerate(zs):
        #             u[:, index] = torch.where(
        #                 u[:, index] <= z, (1 / (j + 1)) * u[:, index], u[:, index]
        #             )
        #     f = self.functions_per_env[env][index]
        #     v[:, index] = f(v, u)
        # v = torch.nan * torch.zeros_like(u)
        ...
        #     f = self.functions_per_env[env][index]
        #     v[:,index] = f(v,u)
        #     if index in self.selected_idx:
        #         for i in range(self.num_label):
        #             zs.append(np.quantile(v[:,index], (i+1)/self.num_label))
        #         for j, z in enumerate(zs):
        #             v[:,index] = torch.where(v[:,index] <= z, np.sqrt(self.var_dict[j])*v[:,index], v[:,index])
        # return v
        v = torch.nan * torch.zeros_like(u)
        for index in self.topological_order:
            zs = []
...
class LinearSCM(MultiEnvLatentSCM):
    ...
    def __init__(
        ...
        """
        super().__init__(
            adjacency_matrix, latent_dim, intervention_targets_per_env, num_blocks, selected_idx, observed_idx
        )
