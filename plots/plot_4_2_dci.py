...
def generate_combined_violin_plots(result_dict, DGP_list):
    ...
    combined_fig, axes = plt.subplots(1, 3, figsize=(9, 3), sharey=False)
    ...
    for idx, (plot_name, result_list) in enumerate(result_dict.items()):
        ...
        gin = pd.concat(data[plot_name]['GIN_synthetic'])
        ivae = pd.concat(data[plot_name]['iVAE_synthetic'])
        ours = pd.concat(data[plot_name]['ours_synthetic'])
        ...
        ax.set_xlabel('')
        ax.set_ylabel('')
    plt.savefig(str(_SCRIPT_DIR / 'combined_architeccture.pdf'), bbox_inches='tight', pad_inches=0.02)
    plt.show()
...
if __name__ == "__main__":
    ...
    # }
    result_dict = {
    "a": ['ours_synthetic', 'GIN_synthetic', 'iVAE_synthetic'],
    "b": ['ours_synthetic', 'GIN_synthetic', 'iVAE_synthetic'],
    "c_real": ['ours_synthetic', 'GIN_synthetic', 'iVAE_synthetic'],
    }
    DGP_list = ['a', 'b', 'c_real']
