...
def mean_corr_coef_np(x, y, method='pearson'):
    ...
    d = x.shape[1]
    if method == 'pearson':
        cc = _cross_corrcoef_np(x, y)
    elif method == 'spearman':
        cc = _cross_corrcoef_np(
            np.apply_along_axis(rankdata, 0, x),
            np.apply_along_axis(rankdata, 0, y),
        )
    else:
        raise ValueError('not a valid method: {}'.format(method))
    cc = np.abs(cc)
...
def _cross_corrcoef_np(x, y):
    ...
    y_norm[y_norm == 0] = np.inf
    return (x / x_norm).T @ (y / y_norm)
