...
base = Path(__file__).resolve().parent / "causal_data"
train_dir = base / "pendulum" / "train"
test_dir = base / "pendulum" / "test"
...
for i in range(-40,44):#pendulum
    for j in range(60,148):#light
        ...
        plt.axis('off')
        if count == 4:
          plt.savefig(test_dir / f"a_{int(i)}_{int(j)}_{int(shade)}_{int(mid)}.png",dpi=96)
          count = 0
        else:
          plt.savefig(train_dir / f"a_{int(i)}_{int(j)}_{int(shade)}_{int(mid)}.png",dpi=96)
        plt.clf()
        count += 1
