...
class ParamMultiEnvCausalDistribution(MultiEnvCausalDistribution):
    ...
    def multi_env_log_prob(
        self, z: Tensor, e: Tensor, intervention_targets: Tensor, observed_idx: list
    ) -> Tensor:
...
def make_learnable_function(parents):
    """
    Creates a learnable function for each node, which is conditioned on its parents.
    This can be an MLP or any other type of function.
    """
    if len(parents) == 0:
        # No parents, just return a linear layer
        return nn.Linear(1, 1)
    else:
        # MLP for combining parent information
        # return nn.Sequential(
        #     nn.Linear(len(parents)+1, 256),
        #     nn.ReLU(),
        #     nn.Linear(256, 1)
        # )
        hidden_dim=256
        return nn.Sequential(nn.Linear(len(parents)+1, hidden_dim),  
                nn.ReLU(),                              
...
class GraphPrior(nn.Module):
    def __init__(self, adjacency, latent_dim):
        super(GraphPrior, self).__init__()
        self.latent_dim = latent_dim
        self.adjacency = adjacency
        self.dag = nx.DiGraph(adjacency)
        self.topological_order = list(nx.topological_sort(self.dag))


        # Create learnable functions for each node
        self.base_functions = nn.ModuleList()
        for index in range(self.latent_dim):
            parents = torch.tensor(
                list(self.dag.predecessors(index)), dtype=torch.int64
            )
            f = make_learnable_function(parents)
            self.base_functions.append(f)
        
        # Map each node to its learnable function
        self.functions_per_node = {}
        self.setup_functions_per_node()

    def setup_functions_per_node(self):
    ...
    def forward(self, z, idx_list):
        ...
        for index in idx_list:
            f = self.functions_per_node[index]
            # Fixed implementation convention: graph predecessors select the
            # learned-coordinate slots passed to this node's predictor.
            parents = list(self.dag.predecessors(index))
            if len(parents) > 0:
                parent_values = z[:, parents]
                input_tensor = torch.cat([parent_values, z[:, index:index+1]], dim=1)
            else:
                v[:, index] = z[:, index]
                continue
            v[:, index] = f(input_tensor).squeeze(-1)
        return v[:, idx_list]
