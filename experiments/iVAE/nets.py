...
class iVAE(nn.Module):
    ...
    def encoder_params(self, x, u):
        ...
        g = self.g(xu)
        logv = self.logv(xu)
        return g, logv.exp().clamp(min=0.01, max=10000)
    ...
    def prior_params(self, u):
        logl = self.logl(u)
        return self.prior_mean, logl.exp().clamp(min=0.01, max=10000)
...
class DiscreteIVAE(nn.Module):
    def __init__(self, latent_dim, data_dim, aux_dim,
                 n_layers=2, hidden_dim=20, activation='lrelu', slope=.1, device='cuda'):
        super().__init__()
        self.data_dim = data_dim
        self.latent_dim = latent_dim
        self.aux_dim = aux_dim
...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim, data_dim, decoder=None, encoder=None,
                 n_layers=3, hidden_dim=50, activation='lrelu', slope=.1, device='cuda'):
        super().__init__()
        self.data_dim = data_dim
        self.latent_dim = latent_dim
        self.hidden_dim = hidden_dim
