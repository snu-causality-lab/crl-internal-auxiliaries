...
from .mixing_function import (
    LinearMixing,
    NonVolumePreservingMixing,
    MixingFunction,
    NonlinearMixing,
    VolumePreservingMixing,
)
...
def make_multi_env_dgp(
    ...
    mixing: str
        Mixing function. Options: "linear", "nonlinear", "volumepreserving",
        or "nonvolumepreserving". Default: "nonlinear".
    mixing_n_flows: int
        Number of coupling blocks for volume-preserving and non-volume-preserving
        data-generating mixings. Default: 8.
    scm: str
    ...
    elif mixing == "volumepreserving":
        mixing_function = VolumePreservingMixing(
            latent_dim=latent_dim, observation_dim=observation_dim,
            n_flows=mixing_n_flows
        )
    elif mixing == "nonvolumepreserving":
        mixing_function = NonVolumePreservingMixing(
            latent_dim=latent_dim, observation_dim=observation_dim,
            n_flows=mixing_n_flows
        )
    else:
